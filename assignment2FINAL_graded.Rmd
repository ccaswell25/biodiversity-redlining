...
**-10 codes does not run**

```{r }
#Notes for me:
#D2_PWDIS =	Wastewater discharge EJ Index
#D5_PWDIS = 	Wastewater discharge Supplemental Index

#Calculating the 95th percentile
la_percentile_95 <- quantile(la_country$D2_PWDIS, probs = 0.95, na.rm = TRUE)

# Identify census block groups above the 95th percentile
above_95 <- la_county %>%
  filter(D2_PWDIS > la_percentile_95)

# Create a centroid for each census block group
above_95_centroids <- st_centroid(above_95)

#Create a map:
ggplot() +
  geom_sf(data = la_county, aes(fill = D2_PWDIS)) +
  geom_sf(data = above_95_centroids, color = "red") +
...
la_county_lowinc <- la_county %>%
 ...
 (nrow(la_county_lowinc) / nrow(la_county)) * 100

# 3.2 % of the census block groups have less than 5% of the population considered low income. 
```
...
**-2 wrong column for % low income**

```{r }
summary_redlining_data <- holc_grade_join %>%
  group_by(holc_grade) %>%
  summarize(mean_perclow_income = mean(P_LOWINCPCT, na.rm = TRUE),
